...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // ...
  std::queue<TreeNode*>nodesNeedToAdd;
  nodesNeedToAdd.push(rootNodePtr);
  while(!nodesNeedToAdd.empty()){
    TreeNode* currentNode = nodesNeedToAdd.front();
    nodesNeedToAdd.pop();
    results.push_back(currentNode->data);
    for(auto child: currentNode->childrenPtrs){
      nodesNeedToAdd.push(child);
    }
